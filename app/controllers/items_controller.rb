  	...
  	unless session.has_key? :user
	  	...
	  	unless params.has_key? :item
	  		...
	  		if not item.has_key? :postcode or item[:postcode].nil? or item[:postcode] == ""
	  			...
	  			redirect_to new_item_path
	  			return
	  		elsif not item[:postcode].match /^([A-PR-UWYZ]([0-9]{1,2}|([A-HK-Y][0-9]|[A-HK-Y][0-9]([0-9]|[ABEHMNPRV-Y]))|[0-9][A-HJKS-UW])\ [0-9][ABD-HJLNP-UW-Z]{2}|(GIR\ 0AA)|(SAN\ TA1)|(BFPO\ (C\/O\ )?[0-9]{1,4})|((ASCN|BBND|[BFS]IQQ|PCRN|STHL|TDCU|TKCA)\ 1ZZ))$/i
